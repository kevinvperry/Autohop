...
| **Podcast Settings** | `SubscriptionSettingsView` | Page | Per-podcast configuration — playback speed, trim silence, auto-archive rules, notifications, Download Filters entry point, and feed exclusion from auto-refresh. Start skip / End skip share one rounded glass card matching the neighbouring Automation section, with the same padding and inset divider; the controls retain fixed minus/plus capsule buttons, compact minute/second text, and debounced persistence so scrolling and live playback stay stable. Also reachable from the gear in an expanded **Up Next** row, which closes Up Next and opens this page (presented as a sheet) in its place. |
...
| **Player** | `PlayerView` | Page (modal overlay) | Full-screen now-playing screen. Shows artwork, title, scrubber, playback controls, chapter list, plus sleep timer, AirPlay, share, and archive actions. On open, the scrubber immediately reflects the restored playback position for a partially played episode rather than waiting for a later tick. A Sleep Schedule indicator pill (bed icon + minutes to the next prompt) appears in the top bar only while inside the active-hours window and pushes the Sleep Schedule page. While a "still listening?" prompt is live a full-screen overlay shows an oversized "Still Listening" confirm button. |
...
| **Listening History** | `ListeningHistoryView` | Page (Menu or App Settings) | Log of all episodes listened to, with duration and date. Grouped by time period. Minimum 60s playback threshold. Resolved rows use the same four swipe positions and behavior as Podcast Detail: Play / Play Next on the leading edge, and a state-driven Download/Archive/Unarchive action plus Play Last on the trailing edge. Actions download first when required, and the standard animated purple progress bar appears beneath a row during download. |
