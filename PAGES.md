...
| **Episode Share** | `EpisodeShareSheet` | Adaptive sheet (from Player or Episode Detail) | Previews the rendered episode card and shares a validated publisher episode page when available; otherwise shares card + details without exposing the enclosure. Offers Copy Link only for safe resolved pages. |
| **Podcast Share** | `PodcastShareSheet` | Adaptive sheet (from Podcast Detail or Podcast Settings) | Shares the podcast's artwork, title, creator and description. It never substitutes the newest episode or exposes the RSS feed URL. |
