...
For Version 1.6 the widget declares both iPhone and iPad device families,
matching the containing app and its Designed-for-iPad Mac availability.
The iOS-family Version 1.6 release was submitted on 30 August 2026. Version 1.6
is now closed and future page/navigation changes belong in VERSION_1.7.md.
...
## Podcast Replay — Version 1.7

- **Podcast Settings → Podcast Replay:** dedicated section immediately above Download Feed Filters, with its own iPad/Mac sidebar shortcut. The schedule editor provides starting episode/search, Start date (Continue from when active), Binge Mode (hides pace controls and uses an undated preview), Daily / Weekdays / Choose Days when scheduled, multiple release times, an immediately saved shared Episode Limit picker, catch-up notification preference, filter preview, concise sync/device notes, Enable/Save and Turn Off. Choose Days reveals weekday buttons; Add another time reveals extra time controls. There is no separate compatibility section or acknowledgement.
- **Podcast Replay → Change Starting Episode:** searchable local catalogue, oldest first, with release dates and full-row selection; returns the chosen episode to the unsaved draft. The picker retains the mini-player.
- **Episode Detail → Listen From Here:** opens the same editor with that episode selected. For an already enabled session, its start remains fixed; turn off before choosing a new start.
- **Subscriptions:** additive Replay pill next to lifecycle status; remains visible while enabled and waiting.
- **Individual Podcast:** Replay pill and Manage Replay link open the editor.
- **Up Next:** released episodes retain their slots while local media downloads; waiting/downloading/retry labels explain availability. Normal swipe actions remain.
- **Caught-up notification:** opens the editor; Keep Schedule / Turn Off actions are checked against the notification's session identity. The editor retains the choice if permission is denied.
- The editor retains `miniPlayerBar(subscriptionID:)`. Apple TV consumes the queue and records listening outcomes; schedule editing belongs to iOS/iPadOS/Designed-for-iPad Mac.
...
> manual Form scrolling updates the selected shortcut from the native list's
> content offset and top reading position, independent of SwiftUI header
> creation order.
...
## Mac navigation and mini-player coverage — 6 September 2026

Both Player → Menu and Subscriptions → Menu inject the shared app environment
inside the presentation closure, including dependencies for Menu descendants.

Application menus route to existing pages through AppRoutingCoordinator and
RootView; Help → Keyboard Shortcuts is DesktopShortcutsView. File import/export
opens Settings' existing workflows. Sleep Timer and Audio Controls present over
the current page. Back uses the visible page's dismiss context. Contextual show
and episode commands use the currently displayed identity; navigation commands
are disabled while a modal is open. See [Mac stage one](Docs/MAC_MENU_STAGE_1.md).

Stats → Top Shows → Show All is TopShowsListView, a list of up to 50 **shows**,
not Discover's Top Episodes. It and the other previously missing destinations
now dock the shared mini-player; see the [coverage audit](Docs/MINI_PLAYER_AUDIT_2026-09-06.md)
for all fixes and intentional presentation exceptions.
...
| **Stats** | `StatsView` | Page (inside Menu, or direct from recap notification) | Listening stats over a selectable period (7 Days / displayed month / displayed year / Lifetime — **7 Days intentionally means the current Monday-to-now calendar week**, and is not a rolling window; the selected month and year pills follow the **This / Last** bar, e.g. "July" → "June" and "2026" → "2025"). A distinct solid **This / Last** bar below the pills switches Week/Month/Year to the previous concluded period (hidden on Lifetime or when that period has no data, except recap notification deep links still open their intended Last view). Sections: time listened, time saved, episodes finished, all-time streak, listening heatmap (or monthly trend chart), 24-hour listening clock, top shows, Shows You're Drifting From (This mode only), data coverage, Stats Health, and time-saved breakdown. Time-based figures use backend-confirmed rendered wall time on iOS and tvOS, split across actual local hour/day boundaries. Top-show data uses canonical feed identity so unsubscribe/resubscribe cycles remain unified; immutable episode outcomes survive capped resume history and later archive/status updates. Imported legacy totals are included only when the range wholly contains their known interval, and partial legacy or forward-only metric coverage is disclosed. The privacy footer only claims iCloud storage while Sync is enabled. Download-Filter Skipped episodes are excluded from drifting-show calculations; manual downloads remain eligible. A bell button in the toolbar opens **Listening Recaps**. |
| **Sleep Schedule** | `SleepScheduleView` | Page (Menu or Player indicator) | Dark glass settings cards with immediate saving, saved-window summary and expandable help. Recurring sleep timer: on/off toggle, active-hours window (may span midnight), and prompt duration (10/15/20/40/60 min or End of Episode). During the window, after the duration a soft chime asks "Are you still listening?" over the continuing playback — any transport command confirms; no response within 60 s fades playback out and rewinds to where the chime started. The end of Active Hours is a hard boundary: pending countdowns and live prompts are dismissed, their notification is cleared, and ordinary playback continues without further prompts. |
| **Menu** | `MenuSheetView` | Sheet | Slide-up menu from the Subscriptions toolbar. The single gateway to Discover (top item — dismisses the Menu and pushes the Discover page), Stats, Sleep Schedule, Listening History, Downloads, App Settings, and Support (last item). Its background and links retain the native grouped palette; Sleep Schedule now uses glass settings cards. The root Menu reserves its lower region for a rich responsive Mini Player whenever an episode is loaded: the links scroll above it, its closely grouped transport controls operate playback, and tapping its remaining surface returns to the permanent Player. Pushed Menu destinations replace this rich card and use the standard persistent mini-player. (Find Podcasts lives behind the + button only.) |
...
> **Public-page parity (30 August 2026):** The website product and Support pages
> cover CarPlay, Widgets, Play Instant, Listening Recaps, Volume Adjustment and
> Mono Audio. The website privacy and Intelligence pages remain supporting
> public documents rather than in-app navigation destinations.
...
The Player's centre audio-output selector displays the active system route name
(including personalised AirPods names), truncates unusually long names instead
of degrading to an icon-only control, and refreshes across AVAudioSession's
short route-settling window.

Both the persistent page Mini Player and the larger Main Menu player use the
same return contract: tapping their card surface clears pushed navigation and
selects the Player's Playing pane, never a previously retained Details or
Chapters pane.
...
1. **Pushed page** — exactly one Back control top-left. Pages with branded Back use `.appNavigationBackButton()`: native Back on iOS 27 and later, and the existing `chevron.left.circle.fill` with ambient SwiftUI dismiss on older systems. Pages already using native Back retain it. Back removes the nearest destination and preserves nested parents; a child must never directly mutate RootView's outer path. Subscriptions retains its Menu control. Podcast Detail and Podcast Settings explicitly retain their dismiss control when presented as modal navigation roots from Player. `MiniPlayerBar` docks at the bottom (hidden during Subscriptions reorder mode); tapping it deliberately clears the complete path to the Player.
...
### Navigation audit evidence — 20 September 2026

<!-- AI CONTEXT — The route contract above follows the shared Back policy;
source-audit breadth does not imply every route received device interaction QA. -->

[The iOS 27 navigation audit](Docs/IOS27_NAVIGATION_BACK_AUDIT.md) records all
reviewed surfaces, 18 migrated destination implementations, modal-root exceptions,
simulator results and outstanding physical-device checks. Keep this route map
aligned with that policy when adding pushed pages or presentation roots.
